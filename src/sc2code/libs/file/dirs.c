...
mkdirhier (const char *path)
{
	...
	while (1)
	{
		...
		*ptr = '\0';
		...
		if (stat (buf, &statbuf) == -1)
		{
#ifndef PSP /* TODO: Debug on PSP, some directories don't like to be stat'ed */
			if (errno != ENOENT)
			{
	...
	{
#ifdef PSP_DEBUG
		log_add (log_Error, "Creating directory %s\n", buf);
#endif

...
#ifdef PSP
	char driveName[10]; /* Max of 9 characters for name due to \0 at end */

	if (pDirHandle->extra->extra->upDir == NULL) {
		// Top dir. Contains only drive letters.
		int len = isPspDrive(name);
		if (len==0 || len>9)
			return NULL;
		memcpy(driveName,name,len);
		driveName[len] = '\0';
...
#elif defined PSP
			// Drive name still needs to follow follow.
			gPDir->extra->cachedPath = uio_malloc(1);
			gPDir->extra->cachedPath[0] = '\0';
#else

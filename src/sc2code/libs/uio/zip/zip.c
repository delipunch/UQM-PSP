...
int zip_updateFileDataFromLocalHeader(uio_Handle *handle,
		zip_GPFileData *gPFileData, uio_FileBlock *fileBlock);
#endif
...
#define EXTRA_LENGTH_PADDING    (256)
    /* Ensure we allocate a file block that contains all of the compressed file
       including filename and extra header data */
	
	handle = uio_malloc(sizeof (zip_Handle));
	uio_GPFile_ref(gPFile);
	handle->file = gPFile;
	handle->fileBlock = uio_openFileBlock2(pDirHandle->pRoot->handle,
			gPFile->extra->headerOffset, gPFile->extra->compressedSize+EXTRA_LENGTH_PADDING);
	...
	if (gPFile->extra->fileOffset == -1) {
		...
		if (zip_updateFileDataFromLocalHeader(pDirHandle->pRoot->handle,
				gPFile->extra, handle->fileBlock) == -1) {
			// errno is set
            uio_GPFile_unref(gPFile);
            uio_closeFileBlock(handle->fileBlock);
		return NULL;
	}
		// We need to add back the headerOffset (offset from the start of the zip file)
		gPFile->extra->fileOffset+=gPFile->extra->headerOffset;
	}
#endif
	...
	handle->baseOffset = gPFile->extra->fileOffset - gPFile->extra->headerOffset;
...
zip_readStored(uio_Handle *handle, void *buf, size_t count) {
    ...
    if (count > ((zip_GPFileData *) (zipHandle->file->extra))->
            uncompressedSize-zipHandle->uncompressedOffset) {
        count = ((zip_GPFileData *) (zipHandle->file->extra))->uncompressedSize-zipHandle->uncompressedOffset;
    }

	numBytes = uio_copyFileBlock(zipHandle->fileBlock,
			zipHandle->uncompressedOffset+zipHandle->baseOffset, buf, count);
	if (numBytes == -1) {
		// errno is set
		return -1;
	}
	...
	while (zipHandle->zipStream.avail_out > 0) {
		if (zipHandle->zipStream.avail_in == 0) {
			...
			numBytes = uio_accessFileBlock(zipHandle->fileBlock,
					zipHandle->compressedOffset+zipHandle->baseOffset,
					((zip_GPFileData *)(zipHandle->file->extra))->compressedSize, //+EXTRA_LENGTH_PADDING,
...
//	fileBlock = uio_openFileBlock(handle);
	fileBlock = uio_openFileBlock2(handle, 0, 0x10000+22+4);
	if (fileBlock == NULL) {
		// errno is set
		goto err;
	}

...
//	uio_debugInteractive(stdin, stdout, stderr);
...
#ifdef PSP
    {
      ...
      mi = mallinfo();
      fprintf (stderr, "mi.arena: %d kb, mi.fordblks: %d kb, ordblks: %d\n", mi.arena/1024, mi.fordblks/1024, mi.ordblks);
      fprintf (stderr, "freememory %d kbytes\n", ((PSP_UQM_HEAP_SIZE_KB*1024) - mi.arena + mi.fordblks)/1024);
    }
#endif

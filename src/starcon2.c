...
PSP_MAIN_THREAD_STACK_SIZE_KB(PSP_UQM_STACK_SIZE_KB);
PSP_HEAP_SIZE_KB(PSP_UQM_HEAP_SIZE_KB);
...
preParseOptions (int argc, char *argv[], struct options_struct *options)
{
	...
	opterr = 0;
#ifdef PSP
    // The PSP version of getopt_long doesn't like it when there are no arguments
    if ( argc > 1 )
#endif
	for (;;)
	{
